...
### Build `0.1.5 (33)` → TestFlight (2026-07-20; `main` HEAD `588382b`) — THE real crash fix; supersedes crashed 31+32
**Builds 31 AND 32 crashed at launch** (SIGABRT on `expo.controller.errorRecoveryQueue` = fatal JS error during bundle eval, NOT a native crash — see docs/TESTFLIGHT-LESSONS.md Issue 5). **Proven root cause via the build-32 `.ips` crash log:** `expo install` had grabbed `@react-navigation/material-top-tabs@7.6.10`, which imports `createScreenFactory` from `@react-navigation/native` — an export missing from the `7.2.2` expo-router SDK 55 pins; the tabs route calls the factory at module scope and expo-router eagerly evaluates all routes at startup → instant crash for everyone. **Fix (`588382b`): pinned `@react-navigation/material-top-tabs@7.4.24`** (newest whose native peer is `^7.2.2`; its imports verified present). Build 32's pager-view 8.0.0→8.0.4 bump was innocent but kept (legit upstream fixes; `expo install --check` will flag it — do NOT downgrade). Build 33 = full 31 content + both pins; built + auto-submitted headlessly. Signed-in cold launch is the critical retest.
