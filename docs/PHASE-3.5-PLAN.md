...
## Open questions for founder (max 5)

- **Trade list expansion:** Balint mentions ~40 trades; we currently seed 12. Should I expand to the full BLS-sourced list now, or ship with 12 and expand later? (Affects onboarding autocomplete UX.)
- **Email provider for verification codes:** We have Twilio for SMS but no email sender configured. Resend or SendGrid? Need API key before chunk 2 can send real verification emails.
- **Photo storage:** Profile and portfolio photo uploads currently save to local filesystem. Should I wire up S3/Cloudflare R2 now (needs bucket + credentials), or keep local for beta and swap later?
- **License auto-verification:** Balint says IL/CA/NY/TX have public lookup APIs. Do you want me to integrate real state board APIs now, or stub with manual-review-only for all states in beta?
- **Trade news in feed (S17):** Balint wants RSS scraping + manual approval queue. Should I build the scraper now, or ship with static placeholder cards and defer the scraper to Phase 4+?
